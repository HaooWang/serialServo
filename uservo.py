...
class UartServoManager:
    ...
    def scan_servo(self, srv_num=254):
        ...
        for servo_id in range(srv_num):
            # 发送ping请求
            self.ping(servo_id, with_logging_info=True)
        if self.is_debug:
    ...
    def set_servo_angle(self, servo_id: int, angle: float, is_mturn: bool = False, interval: float = None,
                        velocity: float = None, t_acc: int = 20, t_dec: int = 20, power: int = 0,
